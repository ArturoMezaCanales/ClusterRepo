...
model_dir = "models_humanoid_SAS"
log_dir = "logs_humanoid_SAS"
...
def train(env, sb3_algo):
    if sb3_algo == 'SAC':
        ...
        model = A2C('MlpPolicy', env, verbose=1, device='gpu', tensorboard_log=log_dir)
    elif sb3_algo == 'DQN':
        # DQN works on discrete action space
        model = DQN('MlpPolicy', env, verbose=1, device='gpu', tensorboard_log=log_dir)
    elif sb3_algo == 'PPO':
        model = PPO('MlpPolicy', env, verbose=1, device='gpu', tensorboard_log=log_dir)
    else:
...
def test(env, sb3_algo, path_to_model):
    ...
    extra_steps = 500 #shows after it falls
    obs_arr = []
    while True:
    ...
    # save the array to a numpy object
